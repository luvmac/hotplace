...
// export default function SearchFilter({ setQ, setDistrict}: SearchFilterProps) {
  ...
  return(
    ...
    placeholder="음식점 검색" 
    className="
    block w-full p-3 text-sm text-gray-800 border border-gray-300 rounded-lg bg-gray-50 outline-none
     focus:border-blue-500 "
    />
    </div>
    <select 
    onChange={(e) => setSearch({ ...search, district: e.target.value })}
     className="bg-gray-50 border border-gray-300 text-gray-800 text-sm md:max-w-[200px] rounded-lg
    outline-none focus:border-blue-500 block w-full p-3">
